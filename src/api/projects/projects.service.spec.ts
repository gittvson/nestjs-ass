...
describe("Projects service", () => {
  ...
  it("should remove user from project", async () => {
    ...
    await projectsService.addUser({ userId: user.id, projectId: project.id });
    await projectsService.removeUser({
      userId: user.id,
  ...
  it("should delete an existing project", async () => {
    ...
    expect(projectsService.findById(project.id)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("should throw if trying to delete a project that doesn't exist", async () => {
    ...
    expect(projectsService.deleteProject("1")).rejects.toThrow(
      NotFoundException,
    );
    expect(projectsService.deleteProject(project.id)).rejects.toThrow(
      NotFoundException,
    );
  });
});
